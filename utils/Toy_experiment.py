...
# import utils.Create_more_metrics as Create_more_metrics
import copy
import os
import pickle
    ...
    for index in important_metrics:
        ...
        if create_more_metrics:
            ...
            for same_metric in more_metrics:
                ...
                if integers2one_hot:
                    module_integers = F.one_hot(module_integers, num_classes=4) # (module_integers, 4, dtype='int8')
            ...
            input_metrics.extend(flattened_adjacent)
            if integers2one_hot:
                module_integers = F.one_hot(module_integers, num_classes=4)# to_categorical(module_integers, 4, dtype='int8')
                module_integers = module_integers.flatten()
            input_metrics.extend(module_integers)
            if additional_metrics:
                if normalization:
